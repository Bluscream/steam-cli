...
The project separates command parsing, non-secret configuration, bounded HTTP transport, Steam Web API discovery, Steam Community session operations, Workshop actions, service-status probing, ASF IPC, SteamCMD management, desktop-client forwarding, and local metadata. Every outbound request goes through one transport layer, so URL validation, redirect suppression, response limits, retry policy, and credential-safe error text are decided once rather than per feature. Cobra supplies standard command help, flag parsing, and shell completion. Mature libraries handle VDF, cross-process file locking, and terminal detection; Go's standard library supplies the protocol and archive primitives. [Cobra](https://github.com/spf13/cobra), [VDF parser](https://github.com/andygrunwald/vdf), [flock](https://github.com/gofrs/flock), [Go standard library](https://pkg.go.dev/std).
...
Implemented now: generic Web API GET/POST including service JSON, catalog discovery/caching, common read helpers; service status probing of web endpoints, connection managers, game coordinators and player counts; Workshop search, subscriptions, favorites, and collection create/edit/delete including membership; SteamCMD bootstrap/run/update/download/workshop; generic ASF API/plugin calls, OpenAPI, bot controls, commands, token reads, and automatic active-user bot scoping; desktop client account switching and profile customization (nickname, bio, real name, vanity URL, privacy) using ASF and Steam Community sessions; hybrid game idling/spoofing via ASF with native Steamworks SDK fallback; forwarding to the desktop Steam client; profiles, diagnostics, completions, local library reading, and Steam ID conversion.

Collection membership, a user's own subscription and favorite lists, and deleting one's own published files are not reachable with a Web API key: the API exposes no method for the first two, and `IPublishedFileService/Delete` is publisher-only. These use the `steamLoginSecure` browser cookie against steamcommunity.com, which is what the Workshop web UI does. Those endpoints are undocumented and may change without notice; they are isolated in one package, each reports its own per-item outcome, and every one of them states plainly when no session is configured rather than reporting a success that changed nothing.

Not implemented: native CM protocol, QR login/token vault, Steam Guard enrollment, standalone depot reconstruction, inventory/trade pagination helpers, websocket log streaming, automatic ASF installation/service management, Proton orchestration or any reimplementation of the client's launch path, or an OS keychain backend. Launching games is delegated to Valve's own launcher through `steamcli client`, which forwards arguments and `steam://` URLs rather than driving the client's internals. Many account operations are already reachable through explicitly requested ASF commands/raw IPC. If direct CM becomes a requirement, evaluate the authentication/protobuf behavior of current SteamKit and steam-user against real accounts before porting individual pieces; importing an old client library is not equivalent to having a tested modern login flow.
