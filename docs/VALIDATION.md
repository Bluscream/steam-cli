...
- Compiled **977 flat SDK methods** against the local reference SDK at `steamworks-rs` revision `6e4a2bcf54f2b520e35db092c5fcf47774f3792a` (913 interface methods plus 64 struct methods).
- Optional native ABI integration test passed with `-race`. It compiles a fake shared library against actual SDK headers and exercises calls, strings, precise uint64 IDs, pointer outputs, struct returns, callback/result lifetime, compiler-derived layouts, game-server accessor selection, invalid arguments, successful/failed idle initialization and control-file shutdown. It does not access a real Steam account.
- Once the desktop client was running, real `SteamAPI_InitFlat` and `ISteamUtils.GetAppID` succeeded for AppID 480. A live persistent session verified logged-in state, exact SteamID representation, a writable beta-name buffer, callback polling and `SteamAPICallCompleted_t` field layout. Account values and SDK diagnostics are not retained here.
- Full `go test -race ./...`, `go vet ./...` and diff checks passed. The Go CLI cross-built for Linux/macOS/Windows, amd64/arm64. Native helper execution was tested on **Linux amd64 only**; game-server mode and background-idle lifecycle were exercised with the native fixture, not a live dedicated server or prolonged real idling session.
- Native function-pointer hooks, C++ callback subclass implementations, and game-engine-owned resources remain application-specific. Complete metadata/dispatch generation is not a claim that every SDK method was executed or is usable for every app/account.

See [SDK usage and limitations](SDK.md). No achievements, stats, workshop content, account settings or game files were modified by the live checks. Brief SDK initialization may update Steam's running-app presence.
