...
**`auto` is the default**: each command prints the clearest form it has — a table, a report, or a reduced ASF value — and falls back to indented JSON when it has no renderer. `table` forces that rendering, `json` and `compact` produce data for scripts, `raw` preserves response bytes from the API, `short` always reduces ASF envelopes to the value behind them, and `csv` emits machine-readable rows (`--with-header` controls the header line); numbers are unformatted there — raw bytes and ungrouped digits — because the point of CSV is to be parsed. Commands that render several tables, such as `status`, emit them one after another, so a consumer should split on the header rows or use `-o json` instead. `-o` is the shorthand, and `--color auto|always|never` governs ANSI output.
